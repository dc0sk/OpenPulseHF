...
impl QrnChannel {
    pub fn new(config: QrnConfig) -> Result<Self, ChannelError> {
        ...
        if config.impulse_rate_hz < 0.0 {
            return Err(ChannelError::InvalidParameter(
                "impulse_rate_hz must be non-negative".into(),
            ));
        }
...
impl ChannelModel for QrnChannel {
    fn apply(&mut self, input: &[f32]) -> Vec<f32> {
        ...
        // Impulsive component: Poisson-distributed spikes per block.
        let expected_spikes =
            self.config.impulse_rate_hz * n as f32 / 8000.0;
