...
| **4 · Transport** | Reliable delivery, segmentation, flow/rate control | ARQ/HARQ retransmission with soft-LLR combining (`harq`, `rate_policy`); SAR segmentation & reassembly (up to 64 KB objects); ACK taxonomy; rate adaptation (`SpeedLevel` / `RateAdapter`) |
