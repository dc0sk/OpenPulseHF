...
# Control-channel security (REQ-CTL)
...
bind; optional on loopback** (`auth_required()` / `REQ-CTL-02`); transmitter-keying commands fail
...
- **System secret store (preferred, `REQ-CTL-03`):** the OS keychain via the `keyring` crate —
...
- **File-based keystore (fallback, `REQ-CTL-04`):** for headless hosts with no usable secret
...
## File permissions (`REQ-CTL-05`, both server and client)
