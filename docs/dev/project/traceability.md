...
- **Tests:** `crates/openpulse-daemon/tests/qsy_lines_are_authenticated.rs` — a stranger's signed
  ...
  fits one frame, and the freshness bound is taken from the shared handshake constant rather than
  re-typed. Five pre-existing tests adapted; one **inverted** —
  `process_received_bytes_with_qsy_req_creates_responder_session` asserted a session WAS created from
  an unsigned REQ, and now asserts it is not. `responder_without_callsign_ignores_qsy_req` was kept
  honest by *signing* its fixture, so it still isolates the §97.119 gate instead of passing because
  the line was refused a step earlier.
- **Test results:** qsy 31 + 18 pass, daemon lib 134 pass, the new file 5/5. Sabotage note: reverting
  the codec produces a **compile** error, not a behavioural failure, so it is not valid evidence for
  a format break — the evidence used instead is the inverted test's own history, where `origin/main`
  fed an unsigned REQ and asserted a session was created. Full `scripts/gate.sh` verdict in the PR.
- **Docs swept (12), and two of them are the more useful finding.**
...
- **Split out:** a stranger's CONREQ re-arms the live session's OTA ACK-MAC key;
  `QsySession::new_initiator()` never receives the configured policy.
