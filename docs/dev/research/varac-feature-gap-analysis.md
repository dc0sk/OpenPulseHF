...
| V20 Auto QSY | In-session QSY w/ accept-reject | **Have** — `openpulse-qsy`: signed + freshness-stamped frames (wired into the daemon at #1252), `QsySession`, scanner, bandplan; daemon-wired (PR #321) | — | — | We exceed VarAC here (signed frames) | Plan keeps it as the in-session mechanism |
