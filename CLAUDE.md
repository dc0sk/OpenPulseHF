...
| `js8-plugin` | `plugins/js8` | JS8-compatible 8-GFSK weak-signal waveform (FF-15); Phase A: `submode.rs` (5 submodes) + `costas.rs` (ORIGINAL/MODIFIED sync arrays) + `modulate.rs` (GFSK tone synthesis) + `ldpc174.rs` (LDPC(174,87) encode) + `crc.rs` (CRC-12, boost-exact) + `frame.rs` (packCallsign/packGrid, Qt-exact) + `tones.rs` (codeword→79 tones) + `message.rs` (payload+flags+CRC→87 info bits) shipped; tables ported from GPL-3.0 JS8Call |
