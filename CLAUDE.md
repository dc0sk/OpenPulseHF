...
**And the deeper reason the numbers were hard to find: our sync word shrinks with baud.** No reference modem allows that — codec2 keeps a ~110 ms PN preamble regardless of payload rate and pays 33 % of the burst for it on datac14. Ours is an alternating run — **32 symbols on BPSK, 16 on QPSK** — so BPSK250 is 124 ms (longer than codec2 pays) while QPSK1000 is 15 ms. But duration is not the variable: an alternating run is two spectral lines, so its **time-bandwidth product is O(1) however long it runs**, and BPSK250 loses to a narrow-filter noise floor (#1060) despite out-lasting the reference. It fails at both ends — BPSK31/63/100 templates exceed `MAX_PREAMBLE_CORRELATION_SAMPLES` (2 048) and are **exempt from the veto altogether**, so #1049 protects exactly one mode. A fixed ~110 ms PN is 880 samples, under the cap for every mode. Tracked in #1062.
