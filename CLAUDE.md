...
**The rectangular QPSK/8PSK pulse is a crossfade — the one-slot demod loses ⅓ of the next symbol.** The "plain" modulator blends adjacent symbols with a raised cosine, so `demodulate_symbols` recovers `sym_k + ⅓·sym_{k+1}`. That `β²=−9.5 dB` ISI floor is invisible to any BER test (45° QPSK margin) but caps every soft consumer — it stalled `mean(|LLR|)` above ~12 dB and floored recovered-symbol EVM at −9.7 dB regardless of SNR. `cancel_crossfade_isi` removes it by stable backward substitution (PR pending; QPSK500 soft-FEC floor was stuck at 0.00 and now decodes). It is *anti-causal* ISI, so the DFE cannot reach it. 8PSK's rectangular path has the same defect.
