...
| Every context the station key signs is bound to a distinct registered domain, and an unregistered signing site cannot compile (REQ-SEC-12) — a hand-maintained list would not do: the inventory rotted three times (issue said 5 contexts, there are 13; my re-derivation missed `OPSE`; review's list missed `OPSP`) | `cargo test -p openpulse-core --no-default-features --lib signing_domain::` + `--lib signing::` + `cargo clippy --workspace --no-default-features --all-targets -- -D warnings` (the `clippy.toml` wall) |
