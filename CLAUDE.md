...
4. **It discriminates against broadband noise only**, so it does **not** retire the condemnation recovery (#1021/#1040) — ablated, removing that recovery fails all three leads, so it is load-bearing. (#1045's `condemned_floor` was the opposite: ablation showed it inert here and harmful on no-template modes, and it has been removed — see the RX-capture-level edge above.) The remaining gap is that the veto only runs where a plugin publishes a `preamble_template`, i.e. BPSK; `QPSK500` still fails on the recorded hot floor, pinned by `a_no_template_mode_decodes_through_a_saturating_floor`. Threshold 0.40 is derived from the decode cliff (BPSK250+Rs on AWGN: ρ 0.561 decodes at −3 dB, 0.455 fails at −5 dB), not from captures. Scope is BPSK, and templates over 2048 samples are excluded, which exempts BPSK31.
