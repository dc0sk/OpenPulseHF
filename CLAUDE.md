...
**FEC short-payload waste.** `FecCodec::encode` always emits a multiple of 255 bytes (4-byte BE length prefix → pad to `block_data` bytes (223 for the default RS(255,223); 191 for `FecCodec::strong()` with ECC_LEN=64) → RS to 255), so a 16-byte payload becomes 255 bytes ≈ 8 s of audio at BPSK31. For ACK frames the engine uses `ShortFecCodec` (`crates/openpulse-core/src/fec.rs:255–317`, exposed via `FecMode::ShortRs`) which emits exactly `payload + ecc_len` bytes. Extending `ShortRs` to variable-length **data** frames is non-trivial: the codec has no length prefix and no block boundary, so the receiver cannot trim the demodulator's trailing bytes — wiring it for data needs a small on-air length envelope. Until then, use BPSK250 or QPSK modes for any test that iterates FEC-encoded data frames at speed.
