...
| A **coded** burst is scanned with slices sized for the CODED frame (#1310 PR1b) — `decode_burst` took its per-attempt slice from the plugin's **raw** `max_frame_samples` (sized for one RS block plus envelope) and hardcoded `FecMode::None` at the decode, so it was an uncoded-only entry point and the ARDOP/KISS front ends had no coded burst path. The boundary is exact and is what the fixture is built on: wire = payload + `Frame::WIRE_OVERHEAD` (10), RS(255,223) is one block iff wire ≤ 223, i.e. **payload ≤ 213 B**. A gate written at 213 B or below passes with the widening deleted and proves nothing — so the discriminating pair is 255 B (two blocks, must decode) against a 200 B **control** (one block, must also decode), both at a **non-zero onset**, since offset 0 is exempt by construction. Sabotage-verified twice, each failing a distinct set: raw sizing fails ONLY the two-block case while the control passes (which is what proves the failure is about sizing, not about coded bursts); re-hardcoding `FecMode::None` fails BOTH coded cases and leaves the uncoded one green | `cargo test -p openpulse-modem --no-default-features --lib burst_decode_sizes_for_the_fec` |
| The KISS TNC accumulates a frame **across reads** and decodes it on a carrier drop (#1310 PR1a) — it called `engine.receive` twice per loop iteration, and `receive` opens a stream, reads ONCE and drops it, so on a callback backend every call saw one 5 ms poll against a seconds-long frame and this TNC could not receive on real audio at all. Structurally invisible to the whole suite because `LoopbackBackend::read` drains its entire buffer, so the buffer IS the frame; the fixture therefore supplies its own chunking backend and asserts `CHUNK < frame.len()` so it cannot go vacuous. The discriminator is chunk SIZE, not alignment (#1247 needed misalignment because that path also decodes per raw chunk; this one decodes nothing until the accumulator flushes). Scope, stated narrowly: this proves accumulation across reads and a flush on carrier drop with a SILENT fixture — **not** that the TNC receives on hardware, which no in-process test reaches. Holding a stream creates the #1007/#1319 obligation, so the second test proves a transmit REOPENS it, observed on the backend's open count (attributable because a read error is the only other thing that drops the stream, and this backend never errors). Sabotage-verified in both directions, each failing its OWN case only: reverting to `receive()` fails the chunked test after 3925 fruitless reads while the drop test passes; deleting `drop_stream()` fails the drop test while the chunked test passes | `cargo test -p openpulse-kiss --no-default-features --test receives_a_chunked_capture` |
