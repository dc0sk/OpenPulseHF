...
3. **Let the correlation PLACE the onset, not merely veto it.** Vetting alone still left 4 condemnations, and instrumenting them showed they were never noise: they sat on the frame's leading *edge*, where partial overlap clears the threshold but the demodulator cannot decode. An accepted candidate now gets a wider second search that snaps the onset and re-settles there.
4. **It discriminates against broadband noise only**, so it does **not** retire `note_condemned` (#1045) or the condemnation recovery (#1021/#1040) — those remain the backstop for anything that correlates but does not decode. Threshold 0.40 is derived from the decode cliff (BPSK250+Rs on AWGN: ρ 0.561 decodes at −3 dB, 0.455 fails at −5 dB), not from captures. Scope is BPSK, and templates over 2048 samples are excluded, which exempts BPSK31.
