...
| The veto reaches the **ladder's faster rungs** — QPSK publishes its own template with its own measured ρ threshold, clearing recorded noise on one side and staying under the decode cliff on the other (#1053) | `cargo test -p openpulse-modem --no-default-features --test qpsk_preamble_correlation_settle` |
...
2. **The residual-frequency grid is bounded from ABOVE by `baud/2`, and this is the load-bearing constant.** Our preamble is 32 *alternating* symbols — a square-wave-modulated carrier with lines at `fc ± baud/2` — so a steady tone correlates to ~0 (the half-symbols cancel) *until* the grid can rotate a line onto plain carrier. Measured ρ of a pure tone: **±20 Hz → 0.017–0.042, ±160 Hz → 0.659, ±450 Hz → 0.696 at every frequency** — above the best real on-air frame (0.654). **This measurement rejects the "obvious" better design** of searching the full acquisition range as a detector and seeding the settle from it (codec2's ordering): a two-spectral-line sync word cannot survive being searched over its own line spacing. Raising that ceiling needs a PN/chirp preamble — a wire-format change.
...
4. **It discriminates against broadband noise only**, so it does **not** retire the condemnation recovery (#1021/#1040) — ablated, removing that recovery fails all three leads, so it is load-bearing. (#1045's `condemned_floor` was the opposite: ablation showed it inert here and harmful on no-template modes, and it has been removed — see the RX-capture-level edge above.) The veto only runs where a plugin publishes a `preamble_template`; that was BPSK alone until **#1053 extended it to QPSK ≤ 500 baud** (see the next edge). Templates over 2048 samples are still excluded, which exempts BPSK31, and 8PSK / the multicarrier modes / QPSK above 500 baud remain energy-only — pinned by `a_no_template_mode_decodes_through_a_saturating_floor`, now on QPSK1000. BPSK's threshold 0.40 is derived from its own decode cliff (BPSK250+Rs on AWGN: ρ 0.561 decodes at −3 dB, 0.455 fails at −5 dB), not from captures — and it is a **BPSK** number, not a receiver-wide one.

**A correlation threshold is a property of the TEMPLATE'S LENGTH, not of the receiver (#1053).** ρ is normalised, so its noise floor is fixed by the template's time-bandwidth product and nothing else — measured over the recorded idle corpus it follows `≈ 6.5/√len` across **both** waveforms (BPSK250 at 992 samples reads 0.205; QPSK125 at 960 reads 0.216 — same length, same number, different modulation). QPSK's preamble is 16 symbols to BPSK's 32, so at equal baud its template is half as long and its noise ceiling √2 higher, and **BPSK's 0.40 sits *below* QPSK500's measured idle-noise ceiling of 0.429**: inheriting it would have corroborated settles on pure noise, silently. So the threshold and grid are published *with* the samples (`PreambleTemplate`, plugin trait 2.0.0) rather than held as engine constants — the API makes "publish a template, inherit someone else's constants" unrepresentable. Three things fell out that are not obvious:
1. **Derive from two measurements per mode and refuse where they do not separate.** The threshold must clear the recorded idle-noise ceiling *and* stay under the weakest ρ that still decodes. QPSK125/250/500 get 0.35/0.45/0.60 at ~1.4–1.8× on each side; **QPSK1000 gets nothing** — 120 samples leave 1.23× either way, with the decode column measured on AWGN (a fade lowers it) and the noise column on one band (a hotter one raises it), so both sides close at once. An energy-only settle is worse than a working veto but better than one that vetoes real frames. More rungs need a longer sync word, i.e. the PN/chirp wire-format change #1052 already points to.
2. **QPSK's preamble is tone-permeable where BPSK's is tone-cancelling, and the test had to change shape, not the code.** BPSK's *alternating* preamble has lines at `fc ± baud/2`, so grid width is load-bearing (a tone: 0.017 at ±20 Hz, 0.659 at ±160 Hz). QPSK's *aperiodic* designed sequence lets a tone find a partial alignment at any width and barely moves (±20 → ±450 Hz: 0.316 → 0.524 for QPSK250). The widen-the-grid falsifier therefore holds for QPSK250 and is **structurally unavailable** for QPSK500; forcing it would have meant fitting a constant to make a test fire. A birdie that passes the veto degrades to the pre-#1049 behaviour; a threshold that rejects decodable frames does not degrade.
3. **Pin BOTH bounds or one of them is unpinned.** Sabotage found it: dropping the threshold to 0.20 fails the noise-ceiling gate but leaves the saturating-floor acceptance case **passing** (an inert veto is exactly the pre-#1049 behaviour, and that behaviour decodes), while raising it to 0.90 — above every decodable frame — broke **nothing at all** until `every_qpsk_threshold_stays_under_its_modes_decode_cliff` was added. A gate on one side of a two-sided derivation reads as coverage and is not.
