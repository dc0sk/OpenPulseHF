...
**🔧 v0.13.0 released (17th of July '26)** — [release notes](https://github.com/dc0sk/OpenPulseHF/releases/tag/v0.13.0). Minor: **a rung of the HF rate ladder was dead on a fading channel, and now works.** `hpx_hf` SL6 (`QPSK250 + RS`) decoded **0% of frames on a routine moderate HF fade at every SNR up to 40 dB** — a coherent, absolutely phase-encoded waveform can't hold a carrier reference through a 1 Hz Doppler fade, so one cycle slip at a fade null ruins the rest of the frame. SL6 is now **`QPSK250-D`**, a differential waveform that encodes each dibit as a phase *increment*, so the fade rotation cancels between adjacent symbols and a slip costs one dibit instead of the frame tail — the same immunity BPSK has always had. Decode rate on that fade goes **0.00 → 0.65**, which makes the rung out-throughput the BPSK rung below it instead of being dead weight. Two plausible fixes were measured and rejected first (the 2-pass carrier tracker, and the pilot waveform — both decode 0%). **Interop: SL6's on-air waveform changed, so both ends need v0.13.0** to use that rung; no config or API change. *Prior release (v0.12.2): HARQ soft-combining on the weak-signal receive path.*
