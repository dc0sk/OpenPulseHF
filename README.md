...
**🔧 v0.12.1 released (16th of July '26)** — [release notes](https://github.com/dc0sk/OpenPulseHF/releases/tag/v0.12.1). Patch: internal fixes from the first formal loose-ends audit of the modem core (broadly solid; gaps clustered in the OTA rate-ladder wiring). Highlights: a send to a **silent peer no longer freezes the daemon control loop** for up to ~36 s (bails after two silent ACK windows; a NACKing peer still gets full retries); the **OTA decoder no longer re-runs the notch/AGC front-end per rate candidate** (which could trip a premature QSY); and the daemon **warns instead of falsely logging "applied"** for OTA knobs it doesn't use. No wire/config change; no action to upgrade. *Prior release (v0.12.0): authenticated rate ACK (E7) + verified-peer consolidation (E5).*
