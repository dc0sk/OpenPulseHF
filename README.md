...
**🎉 v0.7.3 released (15th of July '26)** — [release notes](https://github.com/dc0sk/OpenPulseHF/releases/tag/v0.7.3). The `MFSK16` weak-signal waveform is a full **adaptive-ARQ sub-floor rung**: the HF rate ladder (`hpx_hf`) has a robust non-coherent rung *below* BPSK31 (**SL1**) that the link drops to on deep fade and climbs back out of on recovery, answered by a **K=3 union-decoded MFSK16 ACK** (three spaced copies; the sender union-listens where FSK4 dies). The rung was hardened across a three-pass adversarial audit: **v0.7.1** fixed three breaks that stopped it working on real sound-card hardware (contiguous ACK capture, a low-SNR sync aligner, a mis-scaled SNR estimate); **v0.7.2** added anti-babble Nack budgeting and cross-session ACK filtering; **v0.7.3** adds a dual-waveform ACK (leading FSK4 copy + acquisition) so a peer on a different profile still hears the recommendation. Every audit finding is now addressed. No breaking changes.
