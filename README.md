...
| **CE-SSB conditioning for data modes** | Controlled-Envelope SSB (Hershberger W9GR, QEX 2014) — a voice-SSB technique — applied as an adaptive, per-mode TX conditioner that raises average power at fixed PEP on high-PAPR multicarrier modes (OFDM/SC-FDMA). Confirmed **+1.18 dB on-air**, with software *and* on-air SDR spectral checks showing no added splatter. Believed to be the first open-source HF *data* modem to do this. |
