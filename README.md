...
**🔧 v0.14.1 released (17th of July '26)** — [release notes](https://github.com/dc0sk/OpenPulseHF/releases/tag/v0.14.1). Patch: v0.14.0 fixed the HF ladder's *rungs* for a fade, but the rate controller still couldn't **drive** the link up them — on a routine moderate HF fade `hpx_hf` sat pinned on its entry rung at **~5 bps while decoding every frame**. The controller had one way to climb (a measured SNR clearing a ceiling), and on a fade that estimate is uninformative *in principle* — at 31 baud a 1 Hz fade is faster than any usable averaging window, so it flattens to a constant. Worse, a decoded frame with a low reading was answered by dropping *below* the rung that just decoded. Fix: **a decode is evidence, an SNR estimate is a guess, the evidence wins** — climb after three clean decodes regardless of SNR, and never demote on a frame that decoded (demotion belongs on the failure path). On `moderate_f1` @20 dB the link now climbs from SL1 to **SL11 (OFDM)** instead of pinning at the bottom. Also: BPSK gained a fade-robust SNR estimator (it had none), and the link simulator can now transmit the sub-floor rung. Receiver-side only; no wire, config, or API change. *Prior release (v0.14.0): the fade-aware ladder re-seat.* Measured against a *routine* moderate HF fade (Watterson `moderate_f1`, 1 Hz Doppler), most of the ladder did not work at the SNR floors it advertised — and worst of all **`BPSK31`, the rung every session starts on, decoded 0% of frames at 3, 6 and 9 dB**, so a fading link could not reliably get going at all. Two causes, both measured: the weak rungs shipped **uncoded** (BPSK is differentially decoded, so it rides the fade — but a carrier slip still costs symbols, and nothing was there to repair them), and the coherent mid rungs (`QPSK250`/`QPSK500`/`8PSK500`) decoded **0% at any SNR up to 40 dB** and cannot be saved — a fade null slips their phase reference and ruins the frame. Now: the weak rungs are Reed-Solomon coded (**BPSK31 @3 dB: 0.00 → 0.25**, BPSK63 @7 dB → **1.00**), and above SL6 the ladder is **OFDM**, whose cyclic prefix rides the delay spread (**`OFDM52` decodes 0.58/0.75/0.83 at 8/12/16 dB where `8PSK500` decodes 0.00 at all three**). 14 rungs instead of 17, every one measured to decode on a fade. A third fix fell out: the dense OFDM rungs' floors were on the wrong SNR scale and were **never reachable** — a 35 dB link now climbs to the top. **Interop: both ends need v0.14.0** to share a ladder; no config or API change. *Prior release (v0.13.0): differential QPSK for the SL6 fading rung.*
