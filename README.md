...
**🔒 v0.11.0 released (16th of July '26)** — [release notes](https://github.com/dc0sk/OpenPulseHF/releases/tag/v0.11.0). Two more deferred security-audit fixes on the signed-handshake path. **Replay-freshness:** CONREQ/CONACK now carry a signed timestamp and a verifier rejects stale/future-dated/timestampless handshakes (daemon window ±120 s), so a captured handshake can no longer be replayed indefinitely. **SAR-poison resilience:** the fragment reassembler keyed every handshake under one constant slot, so a single crafted fragment could block a legitimate handshake for the whole timeout (and two real handshakes could poison each other by accident); it now keeps conflicting fragment streams as separate candidates so the good one always completes. **Behaviour change:** the daemon rejects a handshake with no timestamp or one outside ±120 s — upgrade both ends together, and keep stations roughly time-synced. *Prior release (v0.10.0): relay envelope origin authentication (E3).*
