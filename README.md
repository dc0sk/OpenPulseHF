...
**🔒 v0.12.0 released (16th of July '26)** — [release notes](https://github.com/dc0sk/OpenPulseHF/releases/tag/v0.12.0). Closes the last two implementable handshake-trust audit findings. **Authenticated rate ACK (E7):** the tiny FSK4 speed-ladder ACK carried no authentication — its only filter was a hash of the cleartext session id, so any listener could forge ACKs and yank a link's data rate around. The handshake now does an ephemeral X25519 key agreement (public keys ride *inside* the Ed25519-signed CONREQ/CONACK, so a MITM can't swap them), and the ACK carries a keyed MAC that fits inside the existing 5-byte frame (no waveform/airtime change). Authentication, not encryption — content stays in the clear (§97.309 compatible). **Verified-peer consolidation (E5):** all verified-identity reads now go through one authoritative per-callsign store. *Prior release (v0.11.0): handshake replay-freshness + SAR-poison resilience.*
