...
**🔒 v0.8.0 released (15th of July '26)** — [release notes](https://github.com/dc0sk/OpenPulseHF/releases/tag/v0.8.0). A **security release** folding in three back-to-back adversarial audits of the handshake/trust, session, and file-transfer subsystems. It fixes **two CRITICAL issues** — a signed-handshake flaw that let a station impersonate any trusted callsign (the frame key was never bound to the trusted key), and a file-transfer flaw that let a small, quota-approved offer write gigabytes to disk — plus a **SEVERE** availability bug where a file send to a silent peer (routine on HF) locked up the transfer subsystem until a restart. Also: a §97.119 fix so a no-callsign daemon never transmits unidentified, fail-closed trust-store loading, per-sender offer verification, and a working QSY trust filter. Some fixes change runtime behaviour (fail-closed trust load; transmit requires a callsign) — see the release notes — but there is **no wire-protocol or config break**. Anyone running the daemon or file transfer should upgrade.
