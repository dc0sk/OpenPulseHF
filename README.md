...
**🔒 v0.15.0 released (18th of July '26)** — [release notes](https://github.com/dc0sk/OpenPulseHF/releases/tag/v0.15.0). Hardening release. A multi-agent audit of the Winlink network stack found it broadly solid — no panics, no memory-safety bugs, no auth bypass — but turned up **one live denial-of-service** (the proposal cap gated the *answer* rather than the *push*, so a peer streaming proposal frames grew an unbounded list, and a flood that never sent the finish frame spun the receive loop forever) and a cluster of ways a broken or hostile peer could hang or balloon the process. Three of them are worth naming. **Two caps that were each correct still left a hole:** 16 MiB per message and 32 messages per session, with nothing enforcing their *product* — ~512 MB of transient memory from ~2 MB of wire. **A timeout that was not a timeout:** `SO_RCVTIMEO` restarts on every partial read, so it bounded the gap between bytes and never the operation — one byte every few seconds held a read open indefinitely, and the code looked correct doing it. **A capability we never had:** the Winlink Type C (LZHUF) path was documented as compatible, but used LHA `LH5` where Winlink uses classic Okumura LZHUF — a *different bitstream*, so it could never have interoperated. Deleted; inbound Type C is now answered `Reject`, an honest "cannot decode" rather than a silently corrupt decode presented as delivered mail. One air-interface change: the weak rungs now get **free `RsStrong`** where it costs no extra RS block (BPSK31 @3 dB: 0.25 → **1.00**), so **update both ends**. *Prior release (v0.14.1): the rate controller learned to climb on decode evidence rather than an SNR estimate that a fade flattens to a constant — `hpx_hf` on `moderate_f1` @20 dB went from pinned at SL1 (~5 bps while decoding every frame) to **SL11 (OFDM)**.*
