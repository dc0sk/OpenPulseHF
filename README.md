...
**🎉 v0.7.1 released (15th of July '26)** — [release notes](https://github.com/dc0sk/OpenPulseHF/releases/tag/v0.7.1). The `MFSK16` weak-signal waveform is a full **adaptive-ARQ sub-floor rung**: the HF rate ladder (`hpx_hf`) has a robust non-coherent rung *below* BPSK31 (**SL1**) that the link drops to on deep fade and climbs back out of on recovery, answered by a **K=3 union-decoded MFSK16 ACK** (three spaced copies; the sender union-listens for either the K=3 or the FSK4 ACK so an SL1 crossing can't desync) where FSK4 dies. **v0.7.1** is a correctness patch: a 4-finder adversarial audit found the v0.7.0 rung didn't actually work on real sound-card hardware (contiguous ACK capture, a low-SNR sync aligner, and a mis-scaled SNR estimate) — all fixed. No breaking changes.
