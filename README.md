...
**🎉 v0.6.0 released (15th of July '26)** — [release notes](https://github.com/dc0sk/OpenPulseHF/releases/tag/v0.6.0). A feature release from the reference-derived requirements track. Headline: **three new PTT backends** — CM108/CM109/CM119 USB-HID sound-card GPIO (`--ptt cm108`, no extra libraries), a Linux GPIO-line backend (`--ptt gpio`, e.g. a Raspberry Pi header pin), and daemon serial `rts`/`dtr` — plus a **transmitter-release RAII guard and an independent PTT watchdog thread** that force-releases even while the command loop is blocked. Also: **hotplug-safe audio-device resolution** (a renamed/reordered card still resolves), a **simultaneous multi-mode receive monitor**, a config gate for the receiver **AGC**, and the new **`MFSK16` weak-signal waveform** — a constant-envelope 16-GFSK sub-floor mode that decodes on deep-fade HF where BPSK31 drops out. No breaking changes.
