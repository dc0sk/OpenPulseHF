...
**🎉 v0.7.0 released (15th of July '26)** — [release notes](https://github.com/dc0sk/OpenPulseHF/releases/tag/v0.7.0). The `MFSK16` weak-signal waveform becomes a full **adaptive-ARQ sub-floor rung**: the HF rate ladder (`hpx_hf`) now has a robust non-coherent rung *below* BPSK31 (**SL1**) that the link drops to on deep fade and climbs back out of on recovery. It's answered by a **robust K=3 union-decoded MFSK16 ACK** (the receiver sends three spaced copies, the sender union-decodes them and union-listens for either ACK style — measured ~99% at 3 dB below the data floor, where FSK4 dies), with a **payload-capacity guard** (an oversized message rides the next rung that fits instead of being dropped) and **HARQ soft-combining** of weak-rung retransmissions. Validated end-to-end across two real daemons. No breaking changes.
