...
**🔒 v0.10.0 released (16th of July '26)** — [release notes](https://github.com/dc0sk/OpenPulseHF/releases/tag/v0.10.0). Closes the last major deferred finding from the security-audit series: **envelope origin authentication for relayed traffic (E3)**. A relay now cryptographically verifies who originated every relay-data frame it forwards — the control-plane envelope's old 16-byte `auth_tag` was never checked, so any station could stamp a frame with any originator id and have it relayed (impersonation). Because a peer id **is** its Ed25519 verifying key, the envelope now carries an optional Ed25519 origin signature verified against `src_peer_id` with no key exchange, and forged/unsigned relay frames are dropped by default. **Behaviour change:** relays reject unsigned relay frames (mesh nodes now sign); **control-plane wire schema → v2** (not interoperable with v1) — upgrade both ends of a mesh/relay link together. *Prior audit (v0.9.0): RX-decode remote-crash + protocol-bridge hardening.*
