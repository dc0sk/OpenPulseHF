...
**🔒 v0.9.0 released (16th of July '26)** — [release notes](https://github.com/dc0sk/OpenPulseHF/releases/tag/v0.9.0). The second **security release** in the audit series: a fresh adversarial sweep of the **RX decode path** and the **network-facing protocol bridges** (ARDOP/KISS/B2F). It fixes a **CRITICAL** remote crash — a crafted transmission could panic any receiver on frequency, because the short-FEC decode path (used by the OTA-ACK listener every receive tick) fed attacker-length-controlled bytes to a Reed–Solomon decoder with a fixed 256-byte buffer. Also: two more receive-path panics (32-bit/WASM), a bounded SAR reassembler, ARDOP OOM + gzip-bomb + proposal-flood fixes on the TNC/Winlink surface, a signed file offer whose filename is now covered by the signature, and a relay originator allow-list. **Anyone running the daemon or a TNC should upgrade.** Minor bump for the file-offer signature wire change; no config break. *Prior audit (v0.8.0): impersonation, disk-amplification, and send-lockup fixes.*
